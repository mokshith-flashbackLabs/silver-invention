...
### Correction, 2026-08-17: what it also failed to do

The paragraph above is right about *table* grants and wrong by omission about
everything a migrator needs in order to create a table in the first place. As
built, `migrator_services` could not migrate at all. Two privileges were missing,
and neither can live in a migration — this repo's runner **is**
`migrator_services`, so it cannot grant them to itself, and both require the RDS
master:
...
- `GRANT CREATE ON DATABASE` (which the bootstrap did apply) permits
  `CREATE SCHEMA`. It does **not** permit creating a table inside `public`, and
  `REVOKE ALL ON SCHEMA public FROM PUBLIC` removed the fallback. The first
  migration died on `CREATE TABLE schema_migrations` with
  `InvalidSchemaName: no schema has been selected to create in`.
- `WITH GRANT OPTION` is load-bearing, not tidiness: migration 0015 grants `USAGE`
...
- `CREATEROLE` because 0001 creates `imageshield_app`, 0015 the four module roles
  and 0016 `imageshield_proxy_ro`. Without it the runner stops at
  `permission denied to create role`. On PG16 this is narrow — a `CREATEROLE`
  role may only alter or drop roles it created — and it is also what makes 0017
  and 0018 work, since granting membership needs `ADMIN OPTION`, which a role's
  creator holds implicitly.

Both are applied in dev and scripted in the services repo at
`docs/deploy/grant-public-schema.sh`, which must run from the host — there is no
route from CloudShell to RDS, by design (§11).

**`migrator_backend` is half-done.** Measured 2026-08-17: it already holds
`USAGE`/`CREATE` on `public` but `rolcreaterole` is **false**. The proxy's
migrations create roles (its 0012/0013 build `imageshield_proxy` and add
`app_backend`/`app_worker` to it), so it will hit `permission denied to create
role` exactly as this repo did. It needs:

```sql
ALTER ROLE migrator_backend CREATEROLE;
```

Not applied here — that is the proxy repo's deploy to run, and CLAUDE.md §3.6
says ask before a cross-boundary write. As of that date the proxy had not
migrated: only `public` and `svc` existed, with no `profile`, `report` or
`shared`.
...
1. **The AI services opt-out policy (D16).** Highest priority of anything on
   ...
   **No face is enrolled until this returns `optOut`.**
   ...
   > **WAIVED for dev, 2026-08-17, by the project owner.** This account is not in
   > an Organization — `describe-organization` returns
   > `AWSOrganizationsNotInUseException` — and `AISERVICES_OPT_OUT_POLICY` exists
   > only at the Organizations level, so the check above cannot pass as the
   > account stands. Asked and answered: *"we dont need to do opt out policy
   > thing."*
   >
   > What that means concretely, recorded so it is a decision and not an
   > oversight: any face submitted to Rekognition from this environment may be
   > retained and used by AWS for service improvement, possibly outside
   > ap-south-1. For this product that content is a person's biometric identity,
   > and in the cases the product exists to serve it belongs to someone whose
   > likeness has already been misused without consent.
   >
   > The gate stays written here because it is still the right gate. To close it
   > later: create an Organization containing this account (a single-account org
   > is enough), enable the `AISERVICES_OPT_OUT_POLICY` type, attach the policy
   > above, and re-run the verification command. Nothing in the services repo
   > needs to change.
   >
   > As of this date the two dev collections (`identity-dev-v1`,
   > `discovered-dev-v1`) exist and are **empty** — no `IndexFaces` has been
   > called against either.
...
9. ~~**The AI opt-out policy is in effect before any enrolment**, dev included (D16).~~
   **WAIVED for dev 2026-08-17 by the project owner** — see §7.3. The account is
   not in an Organization, so the policy cannot be applied at all. Faces enrolled
   from dev may be retained and used by AWS for service improvement. Both dev
...
| **D16** | The AI opt-out policy is in effect before any face is enrolled. | **WAIVED for dev 2026-08-17** — account not in an Organization, so the policy cannot be applied. §7.3 records what it costs and how to close it. Collections empty as of that date. |
