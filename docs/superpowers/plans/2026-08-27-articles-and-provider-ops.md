...
- Services tooling: `ruff check .` / `ruff format .` (line length 100) / `mypy` (strict) / `pytest` (DB tests need `docker compose -f docker-compose.local.yml up -d`; they skip when Postgres is unreachable — run with `REQUIRE_DB=1` when Docker is up so a skip cannot pass for a pass).
...
Services dir: `ruff check .` → clean; `ruff format .`; `mypy` → clean; `pytest tests/test_admin_providers.py -q` → all pass (existing tests send no operator, so behaviour is unchanged).
...
- [ ] **Step 5: Verify** — services dir: `ruff check .`; `ruff format .`; `mypy`; `pytest tests/test_console.py -q` → existing tests pass (the fake returns `{"providers": []}`, which the new `.get` defaults tolerate).
...
- [ ] **Step 2: Verify** — `ruff check .`; `ruff format .`; `mypy` → clean. `pytest tests/test_boundaries.py -q` → green (no phone-shaped literals, no face search, no S3).
...
- [ ] **Step 5: Verify** — `ruff check .` (it will reorder imports if needed — accept with `ruff check --fix .`); `ruff format .`; `mypy` → clean; `pytest tests/test_auth.py tests/test_error_envelope.py -q` → green (route-auth gates walk the router table).
...
- [ ] **Step 6: Verify** — `ruff check .`; `ruff format .`; `mypy`; `pytest tests/test_console.py -q` → existing tests still green. Then a manual smoke: `python -c "from imageshield.console.app import create_app"` imports cleanly.
...
- [ ] **Step 1: Services** — Docker up; `ruff check .`; `ruff format --check .`; `mypy`; `REQUIRE_DB=1 pytest -q`. Every failure is a gap: fix it in the file that owns the behaviour (not by loosening the test), re-run, and commit with a `fix:` message naming the gap.
