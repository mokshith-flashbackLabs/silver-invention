...
_CONFIRMED_HITS_SQL = """
    ...
    SELECT
      ...
      -- A hit the SUBJECT decided (spec 2026-08-21) has already been answered
      -- by the only person we were waiting on, even though the decision lane
      -- writes no `infringement_feedback` row. Without this clause answering
      -- "yes, this is my photo" costs them SCORE_POSTURE_FEEDBACK — a #45
      -- violation on top of the intended Exposure move — and the app asks
      -- them to respond to a hit they have just responded to. The literal
      -- matches `review/store.py`'s `confirm_decided_by = 'subject'`; both
