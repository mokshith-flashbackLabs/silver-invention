-- Reverses 0027: v_person_hits back to 0023's exact text.
--
-- Postgres cannot drop a column through CREATE OR REPLACE VIEW, so this is
-- DROP + CREATE, which destroys the ACL with the object -- 0016's grant to the
-- proxy's read role is restored below, exactly as 0023's down does, or a
-- rollback that stops here leaves the view existing but unreadable by
-- imageshield_proxy_ro.
--
-- COORDINATED DEPLOY, NOT A SOLO ROLLBACK. This service's own /readyz declares
-- `keyed_on` in EXPECTED_VIEWS (src/imageshield/http/svc_contract.py), so a
-- database reverted past 0027 answers 503 until 0027 is reapplied or the
-- declaration is rolled back with it. The proxy does not read the column yet;
-- once it does, dropping it breaks their reader while breaking nothing else
-- here -- the shape 0016 warned about.
