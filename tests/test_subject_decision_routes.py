...
The route's own duties: the 404 oracle, the 409 on conflict, and the closed
two-value decision vocabulary (``extra='forbid'``)."""
...
def test_a_decided_answer_returns_200() -> None:
    client, store = make_client(_outcome("decided"))
...
def test_a_replay_is_200() -> None:
    client, _store = make_client(_outcome("replay"))
...
def test_a_conflict_is_409_and_not_retryable() -> None:
    client, _store = make_client(_outcome("conflict", decision="rejected"))
...
def test_absent_and_not_yours_are_the_same_404() -> None:
    client, _store = make_client(None)
